...
const FakeoutBuilder = (props) => {
  ...
  const convertTokenToInput = (index) => {
    let text = global.state.items[global.state.currentIndex].fakeout[index].value
  ...
  return (
    <section className={`card fakeout-builder ${currentFakePref == "yes" ? "show" : ""}`}>
      <header>Fakeouts</header>
