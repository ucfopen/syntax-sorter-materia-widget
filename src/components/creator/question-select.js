...
const QuestionSelect = (props) => {
	...
	const addNewQuestion = () => {
		if (global.state.items.length >= MAX_QUESTIONS) return
	...
	return (
		<div className="question-select">
			...
			<button className={`select-btn add-new ${global.state.items.length >= MAX_QUESTIONS ? 'disabled' : ''}`} onClick={addNewQuestion} disabled={global.state.items.length >= MAX_QUESTIONS}>+</button>
